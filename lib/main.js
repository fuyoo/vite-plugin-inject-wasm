...
exports.VitePluginInjectWasm = void 0;
...
const VitePluginInjectWasm = (cfg) => {
    let host;
    let server;
    return {
                    ...
                    if (id.indexOf(`${pid}.js`) > -1) {
                        return new Promise(async (resolve, reject) => {
                            if (server != null) {
                                ...
                                code = code.replace(basename(item.where), `${schema}://${host}:${server.config.server.port}${url_path}?v=${v}`);
                                // existed
                                if (exists) {
                                    resolve(code);
                                }
                                    ...
                                    copyFile(from_path, to_path, (err) => {
                                        if (err == null) {
                                            resolve(code);
                                        }
                                        else {
...
exports.VitePluginInjectWasm = VitePluginInjectWasm;
